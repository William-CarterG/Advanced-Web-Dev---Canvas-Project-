...
function ViewGroup({ closeViewGroupModal, id, setGroups }) {
    ...
    const sendNewColors = () => {
        let body = { "background_color": backgroundColor, "text_font_color": textColor } 
        console.log(body);
        startFetch(`courses/${id}/`, 'PATCH', JSON.stringify(body), function(data) {
    ...
    useEffect(() => {
        if (currentPage === 1) {
          ...
          startFetch(`courses/${id}/`, 'GET', null, function(data) {
            setBackgroundColor(data.background_color);
            setTextColor(data.text_font_color);
            setLogo(data.logo);
            console.log(data);
          });
        }
                ...
                <div className="inline-block w-full max-w-xl p-8 my-20 overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl 2xl:max-w-2xl">
                    ...
                    <form className="mt-5">
                        ...
                        <div className='mt-3 flex gap-4'>
                            <div>
                                <h2 className='text-black mb-1'> Color de fondo </h2>
                                <ColorPicker prevColor={backgroundColor} setColor={setBackgroundColor} />
                            </div>
                            
                            <div>
                                <h2 className='text-black mb-1'> Color de texto </h2>
                                <ColorPicker prevColor={textColor} setColor={setTextColor} />
                            </div>
                            <button
                                onClick={(e) => {
                                    e.preventDefault();
